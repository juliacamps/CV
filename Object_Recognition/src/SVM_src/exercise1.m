...
for i=1:1:size(categories,2)
    ...
    for j=1:1:size(usePercent,2)
        fraction = usePercent(j);
        for z=0:1:1
            useHellinger = z;
            for k=1:1:size(Cs,2)
                C = Cs(k);
                ...
                % For Stage E: Vary the image representation - UNCOMENTED FOR STAGE E!
                ...
                % bias = bias*10^50;
                % w
                testScores = w' * testHistograms + bias ; % I think its being used C)
                ...
                elseif fraction == .1
                    ...
                    new_result = struct('Class',category,'PA',info.auc,'CorrectlyRet',...
                        sum(testLabels(perm(1:36)) > 0),'C',C,'fraction',auxFrac,'kernel',kernel);
...
auxStr = cellstr(['Class',' & ','Train Data \%',' & ','Kernel',' & ',...
    'C',' & ','PA','  \\ \hline']);
for i=1:1:length(results)
    aux = results(i);
    auxStr = [auxStr; aux.Class,' & ',num2str(aux.fraction),' & ',...
       aux.kernel,' & ', mat2str(aux.C),' & ',num2str(aux.PA),'  \\ \hline'];
end
auxStr
