...
% Modified by: Julia Camps
    ...
    %% Code here:
    % Here I plot the shapes for all the training samples ontop of the
    % image choosen for this exercise (face 10) (the shapes are not aligned
    % yet)
    figure(3);
    imshow(app_data(:,:,:,1));
	...
	hold on;
    for i=1:size(shape_data,3)
    triplot(shape_triangles, shape_data(:,2,i), shape_data(:,1,i), 'r'); 
    end
	hold off;
    %% End of code here section
    ...
    %% Code here:
    % Here I plot the same previous shapes, but already aligned, so it can
    % be apretiated the information than they are mapping and the different
    % variations apearing in the training dataset
    figure(4);
    imshow(app_data(:,:,:,1));
    hold on;
    for i=1:size(shape_data, 3)
        triplot(shape_triangles, aligned_data(:,2,i), aligned_data(:,1,i), 'r');
    end
    hold off;
    %% End of the code here section
